...
class Robot:
    ...
    def get_distance(self, degree):
        ...
        self.environment.draw_pixel(30, 200, 30, int(self.pos.x + current_x), int(self.pos.y + current_y))
        return math.hypot(current_x, current_y)

    def distance_from_goal(self, pos):
        return math.hypot(pos.x-self.goal.x, pos.y-self.goal.y)
    ...
    def simulate(self):
        ...
        #print(self.angle)
        print(self.distance_from_goal(self.pos))
        return list(highest_entropy[0]) + list(highest_entropy[1])
"""

    def calculate_different_macrostates_g(self, option, speed):
        macrostates = set()
        angle_options = list(range(-self.lookAngle, self.lookAngle+1))

        curr_pos = Vector2(self.pos.x, self.pos.y)  # copy.copy(self.pos) # import copy
        curr_angle = self.angle
        act_dist = self.distance_from_goal(self.pos)
        ...
        for i in range(self.numberOfPaths):
            ...
            for dt in range(self.timeHorizon):
                ...
                if not self.environment.is_free(*curr_pos.as_int()):
                    break
            m = act_dist - self.distance_from_goal(curr_pos)
            if m > 0:  # if the generated point is closer to the goal than the actual current position -> add to the set
                macrostates.add(curr_pos.as_int())
    ...
    def simulate(self):
        ...
        for speed in speed_options:
            different_futures_opt1 = self.calculate_different_macrostates_g(option1, speed)
            different_futures_opt2 = self.calculate_different_macrostates_g(option2, speed)
            if len(highest_entropy[0])+len(highest_entropy[1]) < len(different_futures_opt1)+len(different_futures_opt2):
                highest_entropy = (different_futures_opt1, different_futures_opt2)
                final_speed = max(min(speed, self.maxSpeed), self.minSpeed)
        ...
        d2 = len(highest_entropy[1])
        n = d1 + d2
        #print("option1*math.log(", d1, "/", n, ")+option2*math.log(", d2, "/", n, ")")
        #decision = option1*(d1/n) + option2*(d2/n)
        try:
            decision = option1*math.log(d1/n)+option2*math.log(d2/n)
        except ValueError:
            decision = self.angle
